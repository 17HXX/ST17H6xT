...
void pwm_open_channel(PWMN_e pwmN,gpio_pin_e pwmPin){
		...
		gpio_fmux_set(pwmPin, (gpio_fmux_e)(FMUX_PWM0 + pwmN));
		PWM_ENABLE_CH(pwmN);
...
void pwm_close_channel(PWMN_e pwmN){	
	if(pwm_gpio_map[pwmN] != GPIO_DUMMY){
		gpio_fmux_control(pwm_gpio_map[pwmN],Bit_DISABLE); 
...
void pwm_ch_start(pwm_ch_t ch)
{
	...
	{
		pwm_set_count_val(ch.pwmN,ch.cmpVal,ch.cntTopVal);
		PWM_SET_DIV(ch.pwmN, ch.pwmDiv);
	}
	else
	{
		pwm_init(ch.pwmN,ch.pwmDiv,ch.pwmMode,ch.pwmPolarity);
		pwm_set_count_val(ch.pwmN,ch.cmpVal,ch.cntTopVal);
		pwm_open_channel(ch.pwmN,ch.pwmPin);
