...
void adc_init_r(ADC_MODE_e mode,ADC_CH_e adc_pin,ADC_SEMODE_e semode,IO_CONTROL_e amplitude)
{
    ...
    {     //mannual mode
        ...
        {                                //normal adcpin            
            ...
            {
                //channel select  
				subWriteReg(0x4000f048,8,8,0);       //positive side enable p20
				subWriteReg(0x4000f048,11,11,semode);  //1 set_single end mode    /negative side enable p15
			}
            else  //ch6
            {
                //channel select
				subWriteReg(0x4000f048,11,11,0); //p15
				subWriteReg(0x4000f048,8,8,semode);  //p20
			}
            
            if(adc_pin == ADC_CH6)
            {
                //set pin to analog io     
                gpio_cfg_analog_io(P20,Bit_ENABLE);//P20       
            }
            else 
            {
                //set pin to analog io
                gpio_cfg_analog_io(P15,Bit_ENABLE); //p15      
            }
