...
typedef struct
{
    ...
    uint8_t         pin_state[MAX_KEY_ROWS];//
...
extern void gpioin_set_flag(gpio_pin_e pin);

uint8_t num_key_rows = 0;
uint8_t num_key_cols = 0;
...
int hal_kscan_init(kscan_Cfg_t cfg, uint8 task_id, uint16 event)
{
	...
	num_key_rows = (sizeof(m_kscanCtx.cfg.key_rows)/sizeof(uint8_t));
	num_key_cols = (sizeof(m_kscanCtx.cfg.key_cols)/sizeof(uint8_t));

    kscan_hw_config();
//    JUMP_FUNCTION(KSCAN_IRQ_HANDLER)   =   (uint32_t)&hal_KSCAN_IRQHandler;
	//JUMP_FUNCTION_SET(KSCAN_IRQ_HANDLER,(uint32_t)&hal_KSCAN_IRQHandler);
    pwrmgr_register(MOD_KSCAN, kscan_sleep_handler, kscan_wakeup_handler);
...
static void hal_kscan_config_row(KSCAN_ROWS_e row)
{
    ...
    gpio_fmux_control(row_pin, Bit_DISABLE);
    gpio_pull_set(row_pin,GPIO_PULL_UP_S);
...
static void hal_kscan_config_col(KSCAN_COLS_e col)
{
    ...
    gpio_fmux_control(col_pin, Bit_DISABLE);
    gpio_pull_set(col_pin,GPIO_PULL_UP_S);
...
static void kscan_sleep_handler(void)
{
    ...
    for(uint8_t i=0; i<num_key_cols; i++)
    {
        ...
        gpioin_set_flag(col_pin);
        gpio_pull_set(col_pin, GPIO_PULL_DOWN);
        gpio_dir(col_pin, GPIO_INPUT);
    }
    ...
    for(uint8_t i=0; i<num_key_rows; i++)
    {
        ...
        gpioin_set_flag(row_pin);
        gpio_pull_set(row_pin, GPIO_PULL_UP);
        gpio_dir(row_pin, GPIO_INPUT);
        pol = gpio_read(row_pin) ? POL_FALLING:POL_RISING;
...
static void kscan_wakeup_handler(void)
{
    ...
    for(uint8_t i=0; i<num_key_rows; i++)
    {
        gpio_pin_e row_pin = (gpio_pin_e)KSCAN_ROW_GPIO[m_kscanCtx.cfg.key_rows[i]];
        subWriteReg(&(AP_IOMUX->keyscan_in_en),m_kscanCtx.cfg.key_rows[i],m_kscanCtx.cfg.key_rows[i],0);
        gpio_pull_set(row_pin, GPIO_PULL_UP);//teddy add 20190122
        gpio_dir(row_pin, GPIO_INPUT);
    }
    ...
    for(uint8_t i=0; i<num_key_rows; i++)
    {
        gpio_pin_e row_pin = (gpio_pin_e)KSCAN_ROW_GPIO[m_kscanCtx.cfg.key_rows[i]];
        gpio_dir(row_pin, GPIO_INPUT);
        gpio_polarity_e pol = gpio_read(row_pin) ? POL_RISING:POL_FALLING;
    ...
    pwrmgr_lock(MOD_KSCAN);

    for(uint8_t i=0; i<num_key_cols; i++) //teddy add 20190122
    {
        gpio_pin_e col_pin = (gpio_pin_e)KSCAN_COL_GPIO[m_kscanCtx.cfg.key_cols[i]];
        gpio_pull_set(col_pin, GPIO_PULL_UP);
        gpio_dir(col_pin, GPIO_INPUT);
    }
