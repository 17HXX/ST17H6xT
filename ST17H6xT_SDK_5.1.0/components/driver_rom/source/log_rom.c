...
static std_putc s_std_putc = (std_putc)swu_uart_send_buff;
//static std_putc s_std_putc = NULL;

/*LOG_LEVEL_NONE (0): no log output*/
/*LOG_LEVEL_ERROR (1): only log error*/
/*LOG_LEVEL_DEBUG (2): output debug info and error info*/
/*LOG_LEVEL_LOG  (3): output all infomation*/
volatile uint32_t s_rom_debug_level = DEBUG_LEVEL;
...
void log_printf(const char *format, ...)
{
	...
	if(s_std_putc)
  	log_vsprintf(s_std_putc, format, args);
}
